 ...
 * 
 * @author John Whaley
 * @version $Id: CUDDFactory.java 454 2006-07-17 03:45:42Z joewhaley $
 */
public class CUDDFactory extends BDDFactory {
    ...
    public BDD load(String filename) {
        long val = load0(filename);
        return new CUDDBDD(val);
    }
    private static native long load0(String filename);

    /* (non-Javadoc)
     * @see net.sf.javabdd.BDDFactory#save(java.lang.String, net.sf.javabdd.BDD)
     */
    public void save(String filename, BDD b) {
        save0(filename, ((CUDDBDD) b)._ddnode_ptr);
    }
    private static native void save0(String filename, long b);
    ...
    public void reorder(ReorderMethod m) {
        if (m == BDDFactory.REORDER_SIFT) {
            reorder0(0);
        } else if (m == BDDFactory.REORDER_SIFTITE) {
            reorder0(1);
        } else {
            throw new UnsupportedOperationException();
        }
     ...
     */
    public void autoReorder(ReorderMethod m) {
        if (m == BDDFactory.REORDER_SIFT) {
            autoReorder0(0);
        } else if (m == BDDFactory.REORDER_SIFTITE) {
            autoReorder0(1);
     ...
     */
    public void autoReorder(ReorderMethod m, int nextReorder) {
        if (m == BDDFactory.REORDER_SIFT) {
            autoReorder1(0, nextReorder);
        } else if (m == BDDFactory.REORDER_SIFTITE) {
            autoReorder1(1, nextReorder);
        } else {
            throw new UnsupportedOperationException();
        }
    }
    private static native void autoReorder1(int method, int nextReorder);
    ...
    public void setReorderTimes(int n) {
        setReorderTimes0(n);
    }
    private static native void setReorderTimes0(int n);
    ...
    private static native void disableAutoReorder0();
    ...
    public double getReorderRuntime() {
        return getReorderRuntime0();
    }
    private static native double getReorderRuntime0();

	public long getRuntime() {
		return getRuntime0();
	}
	private static native long getRuntime0();

	public void setRuntimeLimit(long limit) {
		setRuntimeLimit0(limit);
	}
	private static native void setRuntimeLimit0(long limit);

	public void unsetRuntimeLimit() {
		unsetRuntimeLimit0();
	}
	private static native void unsetRuntimeLimit0();

    /* (non-Javadoc)
     * @see net.sf.javabdd.BDDFactory#reorderVerbose(int)
     */
    public int reorderVerbose(int v) {
        return getNextReorder0();
        // TODO Implement this.
        //throw new UnsupportedOperationException();
    }
    private static native int getNextReorder0();
    ...
    public static final String REVISION = "$Revision: 454 $";
