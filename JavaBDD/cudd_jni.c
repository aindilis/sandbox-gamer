...
JNIEXPORT void JNICALL Java_net_sf_javabdd_CUDDFactory_done0
    ...
    Cudd_PrintInfo(m, file);
    fclose(file);
    printf("\n=== CUDD Statistics ===\n");
    fflush(stdout);
    system("cat statistics");
    printf("\n=== End of CUDD Statistics ===\n\n");
    fflush(stdout);
    Cudd_Quit(m);
}

JNIEXPORT void JNICALL Java_net_sf_javabdd_CUDDFactory_printStat0(JNIEnv *env, jclass cl) {
    FILE* file;
    file = fopen("statistics", "w");
...
JNIEXPORT jint JNICALL Java_net_sf_javabdd_CUDDFactory_getReorderTimes0(JNIEnv *env, jclass cl) {
    return Cudd_ReadReorderings(manager);
}

JNIEXPORT void JNICALL Java_net_sf_javabdd_CUDDFactory_setReorderTimes0(JNIEnv *env, jclass cl, jint n) {
    Cudd_SetMaxReorderings(manager, n);
}

JNIEXPORT jdouble JNICALL Java_net_sf_javabdd_CUDDFactory_getReorderRuntime0(JNIEnv *env, jclass cl) {
    return (double) Cudd_ReadReorderingTime(manager) / 1000.0;
}

JNIEXPORT jlong JNICALL Java_net_sf_javabdd_CUDDFactory_getRuntime0(JNIEnv* env, jclass cl) {
    return Cudd_ReadElapsedTime(manager);
}

JNIEXPORT void JNICALL Java_net_sf_javabdd_CUDDFactory_setRuntimeLimit0(JNIEnv* env, jclass cl, jlong limit) {
    Cudd_SetTimeLimit(manager, limit);
}


JNIEXPORT void JNICALL Java_net_sf_javabdd_CUDDFactory_unsetRuntimeLimit0(JNIEnv* env, jclass cl) {
...
(JNIEnv *env, jclass cl, jint method, jint nextReorder) {
  switch (method) {
  case 0: // sift
    Cudd_AutodynEnable(manager, CUDD_REORDER_SIFT);
    break;
  ...
  Cudd_SetNextReordering(manager, nextReorder);
}

JNIEXPORT int JNICALL Java_net_sf_javabdd_CUDDFactory_getNextReorder0
(JNIEnv *env, jclass cl) {
  return Cudd_ReadNextReordering(manager);
}

JNIEXPORT void JNICALL Java_net_sf_javabdd_CUDDFactory_disableAutoReorder0 (JNIEnv* env, jclass cl) {
...
JNIEXPORT void JNICALL Java_net_sf_javabdd_CUDDFactory_addVarBlock0 (JNIEnv *env, jclass cl, jint first, jint last, jboolean fixed) {
  printf("creating group from variable %d of length %d\n", first, (last - first));
  fflush(stdout);
  if (fixed)
    Cudd_MakeTreeNode(manager, first, (last - first), 4);
  else
    Cudd_MakeTreeNode(manager, first, (last - first), 0);
}
...
JNIEXPORT jlong JNICALL Java_net_sf_javabdd_CUDDFactory_00024CUDDBDD_replace0
    ...
    d = (DdNode*) (intptr_cast_type) a;
    /*if (d == NULL) {
      printf("result(replace d): %ld\n", result);
      fflush(stdout);
      }*/
    e = (CuddPairing*) (intptr_cast_type) b;
    /*if (e == NULL) {
      printf("result(replace e): %ld\n", result);
      fflush(stdout);
      }*/
    for (n=0; n<varnum; ++n) {
        DdNode* node = e->table[n];
        /*if (node == NULL) {
      printf("result(replace node): %ld\n", result);
      fflush(stdout);
      }*/
